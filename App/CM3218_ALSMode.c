...
#define ALSDATA_TOL			0x00f0	/* get ALSens_tmp Tolerance Range */
...
// [* if no device *] " ??? how to do some-thing ???
	...
	SetALSMode();	//include: set (StbBrigF: Status), (DeBuncSens,CntSensStable,CntBrigStable), (AlsBrigTarg,NewAlsBright), (WireIIC_ALS_INITx)	
...
//	static bit StbBrigF=0;	// Stable Brightenss Flag: 0: ALS_OFF,   1: INDOOR or OUTDOOR
	...
	if(UserData.ucALSMode == ALS_OFF) {
		...
		WriteIICWordSeq(CM3218_DEVICE, CM3218_CMD_00, ALS_INIT2);		//0x1084
		if(StbBrigF == 0) {
			// CM3218_ALSOnInit();
...
//printf("00_CM3218_Init\n");
	...
	// (2). [CONVERTER]:
			//(a)_InDoor: used ALS_INIT2 (Sensivity * 2)
				if(UserData.ucALSMode == INDOOR)
				{ //George_Source
					if( NowAlsSens > 0x07FF )		// 500nits
						AlsBrigTarg = 100;
					else if( NowAlsSens > 0x0737 )	// 451nits
						AlsBrigTarg = 90;
					else if( NowAlsSens > 0x0670 )	// 408nits
						AlsBrigTarg = 80;
					else if( NowAlsSens > 0x05A9 )	// 365nits
						AlsBrigTarg = 70;
					else if( NowAlsSens > 0x04E2 )	// 322nits
						AlsBrigTarg = 60;
					else if( NowAlsSens > 0x041B )	// 278nits
						AlsBrigTarg = 50;
					else if( NowAlsSens > 0x0354 )	// 234nits
						AlsBrigTarg = 40;
					else if( NowAlsSens > 0x028D )	// 187nits
						AlsBrigTarg = 30;
					else if( NowAlsSens > 0x01C6 )	// 143nits
						AlsBrigTarg = 20;
					else if( NowAlsSens > 0x00FF )	//  98nits
						AlsBrigTarg = 10;
					else if( NowAlsSens < 0x00FF )	//  74nits
						AlsBrigTarg = 0;
				}

			//(b)_OutDoor: used ALS_INIT0 (Sensivity * 0.5)
				if(UserData.ucALSMode == OUTDOOR)
				{ //George_Source
					if( NowAlsSens > 0x2FFF )		// 500nits
						AlsBrigTarg = 100;
					else if( NowAlsSens > 0x2AE2 )	// 451nits
						AlsBrigTarg = 90;
					else if( NowAlsSens > 0x25C6 )	// 408nits
						AlsBrigTarg = 80;
					else if( NowAlsSens > 0x20A9 )	// 365nits
						AlsBrigTarg = 70;
					else if( NowAlsSens > 0x1B8D )	// 322nits
						AlsBrigTarg = 60;
					else if( NowAlsSens > 0x1670 )	// 278nits
						AlsBrigTarg = 50;
					else if( NowAlsSens > 0x1154 )	// 234nits
						AlsBrigTarg = 40;
					else if( NowAlsSens > 0x0C37 )	// 187nits
						AlsBrigTarg = 30;
					else if( NowAlsSens > 0x071B )	// 143nits
						AlsBrigTarg = 20;
					else if( NowAlsSens > 0x01FF )	//  98nits
						AlsBrigTarg = 10;
					else if( NowAlsSens < 0x01FF )	//  74nits
						AlsBrigTarg = 0;
				}
				//-------------------------------------------
...
//	printf("Ck_Dvi= %d, AlsSens_tmp= %4x, NowAlsSens= %4x, %5u, AlsBrigTarg=%d, NowAlsBright=%d \n", (USHRT)ckDevice, AlsSens_tmp, (USHRT)NowAlsSens, NowAlsSens, (USHRT)AlsBrigTarg, (USHRT)NowAlsBright );	
//	printf("StbBrigF=%d,  ucALSMode=%d, ucBrightness= %d\n", (USHRT) StbBrigF, (USHRT)UserData.ucALSMode, (USHRT) UserData.ucBrightness);
//	printf("SS1= %d\n", (USHRT) OSD_state );
}	
